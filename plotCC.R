##===================================================================
## Function to draw graphics in Cartisian Coordenate Style 
## -------------------------------------------
...
plotCC = function(x,y,z=NA, xmin=NA, xmax=NA, ymin=NA, ymax=NA, zmin=F, zmax=F, type='l',
   ...
   if (!add){
     ...
     plot(x,y, type=type, axes=F, xlab=xlab, ylab=ylab, xlim=c(xmin,xmax), ylim=c(ymin,ymax), pch=pch,...)
  }else{
     if (type=='l') lines(x,y,...)
      if (type=='p') points(x,y,...)
      par(par.original)
     return(cat(''))
  }
  if(addGrid) grid()
